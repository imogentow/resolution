...
def calc_and_plot(sim_series, ax, snap_id=36):
    ...
    for i, sim in enumerate(sims):
        ...
        ax.loglog(cumulative_mass/tot_mass, color=cm[i], label=sim)
    ax.legend()


def cumulative_masses(*sims, snap_id=36):
    ...
    ax[2].set_xlabel("$N_{\\rm{halos}}$")
    ax[1].set_ylabel("$M_{\\rm{cumulative}} / M_{\\rm{200c},\star}$")
    plt.legend()
    plt.subplots_adjust(left=0.2, right=0.99)
    filename = "cumulative_stellar_mass.png"
    plt.savefig(filename, dpi=300)
    plt.show()


cumulative_masses("R1", "R2", "R3", "R4")
